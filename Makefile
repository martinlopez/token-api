...
run-token-job:
	...
	docker run -i -e TOKENS_DATA_FILE=/app/cmd/function/sync_tokens/pkg/ipfs_cids.csv -e ENV=local -e AWS_ACCESS_KEY_ID=${AWS_ACCESS_KEY_ID} -e AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY} -d token-job:1.0.0

execute-migration:
	docker run -v $(shell pwd)/db/migrations:/flyway/sql flyway/flyway:9.5.1 -url=jdbc:mysql://dev-ifps.cfwo0avpvdh0.us-east-1.rds.amazonaws.com:3306/core -user=admin -password=${DBPASSWORD}  migrate
